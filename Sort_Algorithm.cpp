...
void selectSort(int arr[]) {
    for(int i = 0; i < len - 1; i++) {
        ...
        for(int j = i + 1; j < len; j++) {
            if(arr[j] < arr[min_num]) min_num = j;
        }
...
// 最好：O(nlogn)，其他的数均匀分布在基数的两边，此时的递归就是不断地二分左右序列。
// 最坏：O(n^2) ， 其他的数都分布在基数的一边，此时划分左右序列就相当于成了插入排序。
// 平均：O(nlogn)
...
// 3路快速排序
void quickSort_3way(int arr[], int n) {
    if (n <= 1) return;
    ...
    quickSort_3way(arr, j);
    quickSort_3way(arr + k, n - k);
}
